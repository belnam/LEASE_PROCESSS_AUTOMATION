...
def addToMasterfile(folder_name,new_filename_with_timestamp, new_filename,original_name):
    ...
    if new_filename_with_timestamp or new_filename:
        ...
        if ('stoughton' in new_filename or 'STOUGHTON' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'STOUGHTON' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('xtra' in new_filename or 'XTRA' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'XTRA' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('transport' in new_filename or 'TRANSPORT' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'XL TRANSPORT LLC' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('ride' in new_filename or 'RIDE' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'JOY RIDE' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('canadian' in new_filename or 'CANADIAN' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'CANADIAN' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('bowman' in new_filename or 'BOWMAN' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'BOWMAN' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('premier' in new_filename or 'PREMIER' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
                if 'PREMIER' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("0")
                    break
        elif ('milestone' in new_filename or 'MILESTONE' in new_filename) and ("tolls" in new_filename or "TOLLS" in new_filename):
            for subdepartment in subdepartments:
        ...
        elif ('citation' in new_filename and 'milestone' in new_filename) or ("CITATION" in new_filename and "MILESTONE" in new_filename):
            for subdepartment in subdepartments:
                if 'MILESTONE' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'stoughton' in new_filename) or ("CITATION" in new_filename and "STOUGHTON" in new_filename):
            for subdepartment in subdepartments:
                if 'STOUGHTON' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'xtra' in new_filename) or ("CITATION" in new_filename and "XTRA" in new_filename):
            for subdepartment in subdepartments:
                if 'XTRA' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'transport' in new_filename) or ("CITATION" in new_filename and "TRANSPORT" in new_filename):
            for subdepartment in subdepartments:
                if 'TRANSPORT' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'ride' in new_filename) or ("CITATION" in new_filename and "RIDE" in new_filename):
            for subdepartment in subdepartments:
                if 'RIDE' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'canadian' in new_filename) or ("CITATION" in new_filename and "CANADIAN" in new_filename):
            for subdepartment in subdepartments:
                if 'CANADIAN' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'bowman' in new_filename) or ("CITATION" in new_filename and "BOWMAN" in new_filename):
            for subdepartment in subdepartments:
                if 'BOWMAN' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
                    break
        elif ('citation' in new_filename and 'premier' in new_filename) or ("CITATION" in new_filename and "PREMIER" in new_filename):
            for subdepartment in subdepartments:
                if 'PREMIER' in subdepartment['name']:
                    main_subdepartment_id = subdepartment['id']
                    scantype = int("1")
...
def process_excel_file(file_path):
    ...
    required_columns = {'folder_name', 'new_filename_with_timestamp', 'new_filename', 'original_name'}
    ...
    for index, row in df.iterrows():
        ...
        new_filename_with_timestamp = row['new_filename_with_timestamp']
        new_filename = row['new_filename']
        original_name = row['original_name']

        status, scan_id, scantype = addToMasterfile(
            folder_name,
            new_filename_with_timestamp,
            new_filename,
            original_name
        )
        print(f"Row {index + 1}: Status = {status}, Scan ID = {scan_id}, Scan Type = {scantype}")

process_excel_file('res\ScansReUpload.xlsx')

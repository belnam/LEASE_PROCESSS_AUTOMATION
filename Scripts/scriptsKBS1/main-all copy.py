...
for i in os.listdir(workingDir):
      ...
      with pdfplumber.open(workingDir+i) as pdf:
          ...
          for page in pdf.pages:
            ...
            ctrma = re.findall(r'L.*P.*T.*(BILL)',pageText)
            ...
            e470 = re.findall(r'LIC.*PLA.*TOL.*(STATEMENT)|(License).*P\w+\#.*\w+[A-Z]\w+.*\n+T.*Tr.*\s\d+\/\d+\/.*[:]\w+.*\d+\/\d+.*[$|S]\w.*|(Statement)ID\W+State|(License).*P\w+\#.*\w+[A-Z]\w+.*\n+T.*Tr.*\s\d+\/\d+\/.*[:]\w+.*\d+\/\d+.*[$|S]s\d.*',pageText)
            mtatbym = re.findall(r'(ATTENTION)\:|(ATTENTION)\:.*E',pageText)
            ...
            pocahontas = re.findall(r'(Pocahontas)P\w+',pageText)
            ...
            if page1 :
              invoicenumber = re.findall(r'InvoiceNumber\W+(\w+.*)|Invoice\W+Number\W+(\w+.*)|invoiceNumber\W+(\w+.*)|invoice\W+Number\W+(\w+.*)',pageText)
              for invoice in invoicenumber:
                inv = ''.join(invoice).replace("S1","SI").replace("51","SI")
                if inv.startswith("1"):
                  inv = "SI" + inv[1:]
            ...
            if cfea or currentPage  == "PLATE" or currentPage  == "PLATE":
              currentPage = "PLATE"   
              reflpstate = re.findall(r'(S\w+\d+)\W+(\w+).*([A-Z]{1}\w+)\W+\d+\W+\d+\W+\d+\W+(\d+\W+\d+\W+\d+)',pageText)
            ...
            if mtatbym or currentPage  == "ATTENTION":
            ...
            if paturnpike or currentPage  == "Turnpike":
              ...
              licenseplatestate = re.findall(r'Li\w+P\w+\W+(\w+)\W+(\w+)|Li\w+\W+P\w+\W+(\w+)\W+(\w+)',pageText)
            ...
            if pocahontas or currentPage  == "Pocahontas":
              ...
              lpstate = re.findall(r'Li\w+P\w+\W+(\w+)\W+(\w+)|Li\w+\W+P\w+\W+(\w+)\W+(\w+)',pageText)
              for lp in lpstate:
                rowDict["LP"] = lp[1]
                rowDict["LP STATE"] = lp[0]
              duedate = re.findall(r'D\w+D\w+\W+(\d{2}\W+\w+\W+\w+)',pageText)
              for date in duedate:
                rowDict['DUE DATE'] = date

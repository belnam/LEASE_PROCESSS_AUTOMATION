...
pdfpath = f"C:\\Users\\{username}\\Documents\\RENTALS-PROCESS-AUTOMATION\\AMAZON\\"
# Get the path to the pickle files
pickle_file_path_milestone_account = os.path.join(pickle_folder, "last_processed_timestamp_milestone_account.pickle")


# Define the date to filter emails
filter_date = datetime(2024, 3, 15) 

rental_accounts_file = f"Scripts\\Rental_Accounts.xlsx"
try:
    wb_rental_accounts = openpyxl.load_workbook(rental_accounts_file)
    ws_rental_accounts = wb_rental_accounts.active
    desired_account_email = ws_rental_accounts['A4'].value  
...
if desired_account is not None:
    ...
    if os.path.exists(pickle_file_path_milestone_account):
        try:
            with open(pickle_file_path_milestone_account, "rb") as file:
    ...
    with open(pickle_file_path_milestone_account, "wb") as file:
